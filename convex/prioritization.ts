...
export const updateMarketPriority = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (priceVolatility > 0.1) priorityScore += 30; // 10%+ volatility
    ...
    if (spread < 0.02) priorityScore += 20; // Tight spread = high liquidity
    ...
    if (priorityScore >= 70) priority = "hot";      // Sync every 15 seconds
    else if (priorityScore >= 40) priority = "warm"; // Sync every minute
...
export const updateAllPriorities = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    for (const market of markets) {
      await ctx.runMutation("prioritization:updateMarketPriority", {

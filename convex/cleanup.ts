...
export const cleanupOldData = internalAction({
  ...
  handler: async (ctx) => {
    ...
    const snapshotDeleted = await ctx.runMutation(internal.cleanup.deleteOldSnapshots, { 
    ...
    const scoreDeleted = await ctx.runMutation(internal.cleanup.deleteOldScores, { 
      cutoff: scoreCutoff,
      batchSize: 200
    });
    ...
    const baselineDeleted = await ctx.runMutation(internal.cleanup.deleteOldBaselines, {
      cutoff: baselineCutoff,
      batchSize: 100
    });
    
    console.log(`Cleanup completed: ${snapshotDeleted} snapshots, ${scoreDeleted} scores, ${baselineDeleted} baselines`);
...
export const clearAllData = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const tables = ["events", "markets", "priceSnapshots", "scores", "baselines", "marketSyncState"];

                      ...
                      {aiAnalysis.sources && aiAnalysis.sources.length > 0 && (
                        <div className="space-y-1">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {aiAnalysis.sources
                              // Sort by post ID (newer posts have higher IDs, so reverse for most recent first)
                              .sort((a, b) => {
                                const idA =
                                  a.match(/status\/(\d+)/)?.[1] || '0';
                                const idB =
                                  b.match(/status\/(\d+)/)?.[1] || '0';
                                // Compare as BigInt for large Twitter IDs, reverse order for newest first
                                return Number(BigInt(idB) - BigInt(idA));
                              })
                              .map((source, idx) => {

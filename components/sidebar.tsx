...
interface SidebarProps {
  selectedWindow: TremorWindow;
  onChangeWindow: (w: TremorWindow) => void;
...
export function Sidebar({
  ...
  const [internalWindow, setInternalWindow] = useState<TremorWindow>(selectedWindow);
  const [internalIntensity, setInternalIntensity] = useState(
    selectedIntensity || 'all'
  );

  const windows: { id: TremorWindow; label: string; desc: string }[] = [
    { id: '1440m', label: '24 HOUR', desc: 'Daily trends' },
    { id: '60m', label: '1 HOUR', desc: 'Active shifts' },
    { id: '5m', label: '5 MIN', desc: 'Flash moves' },
    { id: '7d', label: '7 DAYS', desc: 'Weekly shifts' },
    { id: '30d', label: '30 DAYS', desc: 'Monthly regime changes' },
    { id: '1Q', label: 'THIS QUARTER', desc: 'Quarter-to-date' },
    { id: '1y', label: '1 YEAR', desc: 'Yearly repricings' },
  ];

  const intensityFilters = [
    { id: 'all', label: 'ALL', range: 'Show all', color: 'bg-zinc-600' },
    { id: 'extreme', label: 'EXTREME', range: '7.5+', color: 'bg-tremor-extreme' },
    { id: 'high', label: 'HIGH', range: '5.0-7.5', color: 'bg-tremor-high' },
    { id: 'moderate', label: 'MODERATE', range: '2.5-5.0', color: 'bg-tremor-moderate' },
    ...
    <aside className="from-zinc-950/98 flex h-full w-64 flex-col overflow-hidden border-r border-zinc-800/50 bg-gradient-to-b to-black/95">
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="px-5 pt-5">
          <h3 className="mb-4 text-xs font-bold tracking-wider text-zinc-600">FILTER BY INTENSITY</h3>
            ...
            {intensityFilters.map((filter) => (
              <button
                ...
                className={`block w-full text-left transition-all ${
                  internalIntensity === filter.id ? 'border-2 bg-zinc-950' : 'border bg-zinc-950 hover:border-zinc-600'
                }`}
                style={{
                  borderColor:
              ...
              >
                <div className="p-3">
                  <div className="flex items-center gap-3">
                    {filter.id === 'all' ? (
                      <img src="/tremor-icon.svg" alt="Tremor logo" className="h-8 w-8" />
                    ) : (
                    ...
                    <div className="flex-1">
                      <div className="flex items-baseline gap-2">
                        ...
                        <span className={`text-[10px] uppercase ${
                          filter.id === 'extreme' ? 'text-tremor-extreme' :
                          filter.id === 'high' ? 'text-tremor-high' :
                          filter.id === 'moderate' ? 'text-tremor-moderate' : 'text-zinc-500'
                        }`}>
                          {filter.label}
                        </span>
                      </div>
                      <div className="text-[10px] text-zinc-500">
                        {filter.id === 'extreme' ? 'Market earthquake' :
                         ...
                         filter.id === 'low' ? 'Background noise' : filter.range}
                      </div>
                    </div>
                    {internalIntensity === filter.id && (
        ...
        <div className="mt-6 px-5">
          <h3 className="mb-4 text-xs font-bold tracking-wider text-zinc-600">TIME WINDOW</h3>
          <div className="space-y-2">
            {windows.map((window) => (
              <button
                key={window.id}
                onClick={() => {
                  const w = window.id as TremorWindow;
                  setInternalWindow(w);
                  onChangeWindow(w);
                }}
                className={`block w-full text-left transition-all ${
              ...
              >
                <div className="p-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-sm font-bold text-zinc-100">{window.label}</div>
      ...
      <div className="flex-shrink-0 border-t border-zinc-800/50 p-5">
        <div className="mb-1 text-center text-[9px] uppercase tracking-wider text-zinc-600">Powered by</div>
        <div className="flex flex-col items-center gap-0.5">
          <a href="https://polymarket.com" target="_blank" rel="noopener noreferrer" className="text-[10px] text-zinc-400 transition-colors hover:text-zinc-200">Polymarket</a>
          <a href="https://x.ai" target="_blank" rel="noopener noreferrer" className="text-[10px] text-zinc-400 transition-colors hover:text-zinc-200">Grok AI</a>
        </div>
      </div>
    </aside>
  );
